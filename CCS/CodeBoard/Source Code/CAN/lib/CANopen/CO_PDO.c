        ...
        uint8_t                *pIsMultibyteVar)
{
    ...
    subIndex = (uint8_t)(map>>8) & 0xFF;
    dataLen = (uint8_t) map & 0xFF;   /* data length in bits */
...
#else
        for(j=prevLength; j<length; j+=2){
            TPDO->mapPointer[j] = pData;
            TPDO->mapPointer[j+1] = pData++;
        }
...
int16_t CO_TPDOsend(CO_TPDO_t *TPDO){
    ...
    for(; i>0; i-=2) {
        *(pPDOdataByte++) = **(ppODdataByte++)  & 0xFF;
        *(pPDOdataByte++) = **(ppODdataByte++) >> 8 & 0xFF;
    }
