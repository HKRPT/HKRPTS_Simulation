...
void myEPWM_Init(void)
{
    ...
    myEPWMx_Init(EPWM1,TPRD, 0, 0 , 150); // PWM通道初始化
    myEPWMx_Init(EPWM2,TPRD, 0, 0 , 150); // PWM通道初始化
    myEPWMx_Init(EPWM3,TPRD, 0, 0 , 150); // PWM通道初始化
 ...
 *
 */
void myEPWMx_Init(uint16_t port,uint16_t period, uint16_t compareA, uint16_t compareB,uint16_t deadband)
{
    EALLOW;
    ...
    EPwmxRegs[port]->TBCTL.bit.PHSEN        = 0x00;                     // 关闭相移
    ...
    EPwmxRegs[port]->TBPHS.bit.TBPHS        = 0x00;                     // 设置相移
