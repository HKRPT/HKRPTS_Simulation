...
void* CO_OD_getDataPointer(CO_SDO_t *SDO, uint16_t entryNo, uint8_t subIndex){
    ...
    if(object->maxSubIndex == 0U){   /* Object type is Var */
        ...
        if(subIndex==0){
            ...
            return (void*)(((int8_t*)object->pData) + ((subIndex-1) * ((object->length /2) == 0 ? 1:object->length /2)));
        }
    }
    else{                            /* Object Type is Record */
...
uint32_t CO_SDO_writeOD(CO_SDO_t *SDO, uint16_t length){
    ...
    if((ODdata != NULL) && !exception_1003){
        if(length == 4)
        {
            *(ODdata++) = (uint32_t)(SDObuffer[1] & 0xFF) << 8 | (uint32_t)(SDObuffer[0] & 0xFF) << 0;
            *(ODdata)   = (uint32_t)(SDObuffer[3] & 0xFF) << 8 | (uint32_t)(SDObuffer[2] & 0xFF) << 0 ;
        }
        else if (length == 2)
        {
            *(ODdata)    = ((SDObuffer[0]) & 0xFF) << 0;
            *(ODdata)   |= ((SDObuffer[1]) & 0xFF) << 8;
        }
        else if(length == 1)
        {
            *(ODdata)   = *(SDObuffer) & 0xFF;
        }
        else
        {
            while(length--){
    ...
    switch(state){
        ...
        case CO_SDO_ST_DOWNLOAD_BL_SUB_RESP_2:{
            ...
            if((SDO->ODF_arg.ODdataStorage == 0) && (SDO->bufferOffset != 0) && !lastSegmentInSubblock){
                ...
                if(SDO->crcEnabled){
                    SDO->crc = crc16_ccitt((unsigned char *)SDO->ODF_arg.data, SDO->bufferOffset, SDO->crc);
                }
        ...
        case CO_SDO_ST_UPLOAD_INITIATE:{
            ...
            if(SDO->ODF_arg.dataLength <= 4U){
                for(i=0U,j = 0U; i<SDO->ODF_arg.dataLength; i+=2)
                {
                    SDO->CANtxBuff->data[4U + i]      = SDO->ODF_arg.data[j] >> 0 & 0xFF;
                    SDO->CANtxBuff->data[4U + i + 1]  = SDO->ODF_arg.data[j] >> 8 & 0xFF;
                    j++;
                }
        ...
        case CO_SDO_ST_UPLOAD_BL_INITIATE:{
            ...
            if((CANrxData[0] & 0x04U) != 0U){
                SDO->crcEnabled = true;
                SDO->crc = crc16_ccitt((unsigned char *)SDO->ODF_arg.data, SDO->ODF_arg.dataLength, 0);
            }
            else{
        ...
        case CO_SDO_ST_UPLOAD_BL_SUBBLOCK:{
            ...
            if(isNew){
                ...
                if((SDO->ODF_arg.ODdataStorage == 0) && (SDO->ODF_arg.dataLength < (SDO->blksize*7U)) && (!SDO->ODF_arg.lastSegment)){
                    ...
                    /* calculate CRC on next bytes, if enabled */
                    if(SDO->crcEnabled){
                        SDO->crc = crc16_ccitt((unsigned char *)SDO->ODF_arg.data, SDO->ODF_arg.dataLength, SDO->crc);
                    }
